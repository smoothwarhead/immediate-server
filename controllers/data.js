                        ...
                        db.query(queryProfile, [trainerId], async (err, result2) => {
                            if(err){
                                ...
                                res.status(401).json({
                                    profile: [],
                                    logIn: true,
                                    message: "No profile to display at this time"
                                });
                            }
                            if(result2.length === 0){
                ...
                const getTrainerId = "SELECT trainer_id FROM trainer WHERE user_id = ?";
                 ...
                 db.query(getTrainerId, [userId], (err, result) => {
                    ...
                    if(result){
                        ...
                        // queryProfile = "SELECT * from class where trainer_id = ?";
                        ...
                        db.query(queryClient, [trainerId],  (err, result2) => {
                            if(err){
                                ...
                                res.status(401).json({
                                    clients: [],
                                    logIn: true,
                                    message: "No client to display at this time"
                                });
                            }
                            if(result2.length === 0){
                                    ...
                                    if(result1){
                                        ...
                                        db.query(getEquipment, [classId], (err, result2) => {
                                            ...
                                            if(result2){
                                                ...
                                                // const updateValues = [putEquipment.map(equipment => [equipment.item, equipment.id, classId])];
